...
def format_instruction_prompt(user_input: str, user_marker: str, assistant_marker: str) -> str:
    return f"{user_marker} {user_input}\n{assistant_marker} "


def cut_response(text: str, stop_tokens: list[str]) -> str:
    stop_positions = []
...
def extract_assistant_text(output: str, assistant_marker: str, stop_tokens: list[str]) -> str:
    if assistant_marker in output:
        output = output.split(assistant_marker, 1)[-1]
    return cut_response(output, stop_tokens=stop_tokens)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate responses from a fine-tuned GPT V3 checkpoint."
    )
    parser.add_argument("--env", default=None, help="Config environment: development/production/testing.")
    parser.add_argument(
        "--checkpoint",
        default=None,
        help="Checkpoint path (default: <TRAIN_SAVE_DIR>/gpt_finetuneV3.pt).",
    )
    parser.add_argument("--prompt", default=None, help="User prompt. If omitted, starts interactive mode.")
    parser.add_argument("--raw-prompt", action="store_true", help="Use prompt exactly as given, without .user./.assistant. formatting.")
    parser.add_argument("--user-marker", default=".user.", help="User marker used in fine-tune data/prompt template.")
    parser.add_argument("--assistant-marker", default=".assistant.", help="Assistant marker used in fine-tune data/prompt template.")
    ...
    if not args.user_marker:
        raise SystemExit("--user-marker must not be empty.")
    if not args.assistant_marker:
        raise SystemExit("--assistant-marker must not be empty.")
    if args.user_marker == args.assistant_marker:
        raise SystemExit("--user-marker and --assistant-marker must be different.")
    ...
    checkpoint_path = Path(args.checkpoint) if args.checkpoint is not None else (
        Path(str(getattr(cfg, "TRAIN_SAVE_DIR", "reports/checkpoints"))) / "gpt_finetuneV3.pt"
    )
    ...
    print("=" * 80)
    print("GPT Generate FineTune V3")
    print("=" * 80)
    print(f"env={env}")
    if checkpoint_env:
    ...
    print(f"raw_prompt={bool(args.raw_prompt)}")
    ...
    stop_tokens = [args.user_marker, args.assistant_marker]

    if args.prompt is not None:
        prompt_text = (
            args.prompt
            if args.raw_prompt
            else format_instruction_prompt(
                args.prompt,
